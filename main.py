...
async def worker(browser_id: int):
    ...
    async with async_playwright() as playwright:
        try:
            ...
            try:
                await repo.session_commit(task_name)
            except Exception as e1:
                logger.debug(f'repo.session_commit()============{browser_id}')
            try:
